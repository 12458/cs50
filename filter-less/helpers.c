    ...
    */
    ...
    for (int h = 0; h < height; h++) {
        for (int w = 0; w < width; w++) {
            ...
            image[h][w].rgbtRed = (BYTE)round(cum_red / (float)items);
            image[h][w].rgbtGreen = (BYTE)round(cum_green / (float)items);
            image[h][w].rgbtBlue = (BYTE)round(cum_blue / (float)items);
        }
    }
